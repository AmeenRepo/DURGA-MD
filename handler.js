...
/** 
     ...
     * @returns 
     */
    global.sendPoll = (jid, name = '', values = [], selectableCount = 1) => { return global.sendMessage(jid, { poll: { name, values, selectableCount }}) }
...
global.ev.on('messages.update', async chatUpdate => {
        ...
        if (update.pollUpdates && key.fromMe) {
            ...
            if (pollCreation) {
                ...
                try {
                    ...
                    await global.sendMessage(key.remoteJid, { delete: key });
                } catch (error) {
                    console.error("Error deleting message:", error);
                }

                global.appendTextMessage(prefCmd, chatUpdate);
            }
        }
    }
});

global.getPoll = (name = '', values = [], selectableCount = 1) => {
    return global.sendMessage({ poll: { name, values, selectableCount }});
}
